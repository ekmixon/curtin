...
def superblock_asdict(device=None, data=None):
    ...
    for line in data.splitlines():
        ...
        bcache_super[values[0]] = values[1]
...
def _check_bcache_type(device, sysfs_attr, sb_version, superblock=False):
    ...
    sys_block = (
        device
        if device.endswith('bcache')
        else os.path.join(sys_block_path(device), 'bcache')
    )
...
def sysfs_path(device, strict=True):
    ...
    if strict and not os.path.exists(path):
        err = OSError(f"device '{device}' did not have existing syspath '{path}'")
...
def get_cacheset_members(cset_uuid):
    ...
    if os.path.exists(cset_path):
        ...
        members = [os.path.realpath(f"{cset_path}/{bdev}") for bdev in bdevs]
...
def get_cacheset_cachedev(cset_uuid):
    ...
    cachedev = f'/sys/fs/bcache/{cset_uuid}/cache0'
    return os.path.realpath(cachedev) if os.path.exists(cachedev) else None


def attach_backing_to_cacheset(backing_device, cache_device, cset_uuid):
    LOG.info(
        f"Attaching backing device to cacheset: {backing_device} -> {cache_device} cset.uuid: {cset_uuid}"
    )
...
def get_backing_device(bcache_kname):
    ...
    """
    bcache_deps = f'/sys/class/block/{bcache_kname}/slaves'
    ...
    # to a cacheset.
    if len(deps) == 0:
        raise RuntimeError(f'{bcache_kname} unexpected empty dir: {bcache_deps}')

    return next(
        (dev for dev in (sysfs_path(dep) for dep in deps) if is_backing(dev)),
        None,
    )
...
def stop_cacheset(cset_uuid):
    ...
    if cset_uuid.startswith('/sys/fs/bcache/'):
        ...
        cset_device = f"/sys/fs/bcache/{cset_uuid}"
...
def stop_device(device):
    ...
    if not device.startswith('/sys'):
        raise ValueError(f'Invalid device {device}, must be sysfs path')

    if not any(f(device) for f in (is_backing, is_caching)):
        raise ValueError(f'Cannot stop non-bcache device: {device}')
...
def set_cache_mode(bcache_dev, cache_mode):
    LOG.info(f"Setting cache_mode on {bcache_dev} to {cache_mode}")
    cache_mode_file = f'/sys/block/{bcache_dev}/bcache/cache_mode'
...
def validate_bcache_ready(bcache_device, bcache_sys_path):
    ...
    if bcache_sys_path.startswith('/sys/fs/bcache'):
        ...
        if not cache_links:
            msg = f'Failed to find any cache links in {bcache_sys_path}:{sys_path_links}'
            raise OSError(msg)
        ...
        for link in cache_links:
            ...
            if os.path.basename(bcache_device) == target_cache_device:
                ...
                msg = (
                    f'Cache symlink {target_cache_device} '
                    + f'points to incorrect device: {bcache_device}'
                )
        ...
        if os.path.islink(bcache_dev):
            ...
            if os.path.basename(bcache_device) in slaves:
                ...
                msg = (
                    f'Failed to find bcache device {bcache_device}'
                    + f'in slaves list {slaves}'
                )
        ...
        LOG.debug("Failed to validate bcache device '%s' from sys_path"
                  " '%s'", bcache_device, bcache_sys_path)
        msg = f'sysfs path {bcache_sys_path} does not appear to be a bcache device'
    ...
    for attempt, wait in enumerate(retry):
        ...
        try:
            ...
            if os.path.exists(expected):
                ...
                msg = f'bcache device path not found: {expected}'
                LOG.debug(msg)
...
def create_cache_device(cache_device):
    ...
    if os.path.exists(os.path.join(cache_device_sysfs, "bcache")):
        LOG.debug(
            f'caching device already exists at {cache_device_sysfs}/bcache. Read cset.uuid'
        )

        (out, err) = util.subp(["bcache-super-show", cache_device],
                               capture=True)
        LOG.debug(f'bcache-super-show=[{out}]')
        [cset_uuid] = [line.split()[-1] for line in out.split("\n")
                       if line.startswith('cset.uuid')]
    else:
        LOG.debug(
            f'caching device does not yet exist at {cache_device_sysfs}/bcache. Make cache and get uuid'
        )
    ...
    target_sysfs_path = f'/sys/fs/bcache/{cset_uuid}'
...
def create_backing_device(backing_device, cache_device, cache_mode, cset_uuid):
    ...
    if len(holders) != 1:
        err = f'Invalid number {len(holders)} of holding devices: "{holders}"'
        LOG.error(err)
        raise ValueError(err)
    [bcache_dev] = holders
    LOG.debug(f'The just created bcache device is {holders}')
    ...
    if cache_device:
        ...
        if cset_uuid:
            ...
            msg = f"Invalid cset_uuid: {cset_uuid}"
            LOG.error(msg)
            raise ValueError(msg)
